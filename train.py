#-------------------------------------- Imported all the saem libraries------------------
import os  
import argparse
import json  
import torch 
...
def valid(val_loader, network):
    ...
    for batch in val_loader:
        ...
        with torch.no_grad():  # torch.no_grad() may cause warning
            output = network(source_img)   # This is missing .clamp_(-1, 1)	

            # This loss1 is extra here
            loss1 = criterion[0](output, target_img)
            # loss2 = criterion[1](output, target_img)
            loss = loss1
    ...
    return losses.avg, PSNR.avg  # losses.avg, is extra here



#-----------------This method is extra here------------------------------------
...
# ----------------------------------This line is same, But here this append the losss in list-----------
    # build criterion
    criterion = []
...
# ----------------------------------This line is Same, but "last_epoch=begin_epoch - 1" is Extra here---------------------------
    # build scheduler
...
# ----------------------------------This line is Same, but This has not used  below line-------------------------------------------------------------------------
...
#  This line is used inside the path.exists  
    # init SummaryWriter
...
#----------------------------- This line is Extra Here---------------------------------------
    ...
    # This line is missing here and not used 
    #best_psnr = 0

#----------------------------------------- This for epoch is different ---------------------------
